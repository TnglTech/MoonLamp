...
# TODO: Rewrite how this works to make it less janky
    ...
    def reconfigure(self):
        ...
        try:
            ...
            if not self.execute_command(
                    base_command + "set_network " + network_number + " ssid '\"" + self.ssid + "\"'").startswith('OK'):
                raise Exception("Error: Could not set SSID")

            if self.psk is None:
                if not self.execute_command(
                        base_command + "set_network " + network_number + " key_mgmt NONE").startswith('OK'):
                    raise Exception("Error: Could not set key management to NONE")
            else:
                if not self.execute_command(
                        base_command + "set_network " + network_number + " key_mgmt WPA-PSK").startswith('OK'):
                    raise Exception("Error: Could not set key management to WPA-PSK")
                if not self.execute_command(
                        base_command + "set_network " + network_number + " psk '\"" + self.psk + "\"'").startswith(
                        'OK'):
            ...
            self._error_message = ex.message or "There was an error setting the WiFi config"
            print(ex)
            print("Update Failed")
        finally:
            self.execute_command(base_command + "save_config")
            self.execute_command(base_command + "reconfigure")
            return successful, self._error_message
